...
namespace Infrastructure.ProjetoCursosIdioma.Repositories
{
    public class SQLAlunoRepository : IAlunoRepository
    {
        ...
        //Validation
        public async Task<bool> EmailAlreadyUsedAsync(string email, Guid? alunoIdIgnorado = null)
        {
            return await _DbContext.Alunos.AnyAsync(a => a.Email == email &&
            (!alunoIdIgnorado.HasValue || a.Id != alunoIdIgnorado.Value));
        }
        ...
        public async Task<bool> CPFAlreadyUsedAsync(string cpf, Guid? alunoIdIgnorado = null)
        {
