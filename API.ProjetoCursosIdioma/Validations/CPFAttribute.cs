...
namespace API.ProjetoCursosIdioma.Validations
{
    ...
    public sealed class CPFAttribute : ValidationAttribute
    {
        ...
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            ...
            string cpfNormalizado = NormalizadorCPF.Normalize(cpf);

            var validation = new CPFValidator();

            return validation.IsValid(cpfNormalizado) ? ValidationResult.Success : new ValidationResult(ErrorMessage);
        }
    }
}
